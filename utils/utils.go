...
// GenerateUUID creates a 16byte UUID
...
func StringToGuid(guid string) ([]byte, error) {
	...
	for i, part := range parts {
		hexBytes, err := hex.DecodeString(part)
		if err != nil {
			return nil, err
		}

		if i < 3 {
			for j, k := 0, len(hexBytes)-1; j < k; j, k = j+1, k-1 {
				hexBytes[j], hexBytes[k] = hexBytes[k], hexBytes[j]
			}
		}

		parsedBytes = append(parsedBytes, hexBytes...)
	}

	return parsedBytes, nil
}

func GuidToString(bytes []byte) (string, error) {
	if len(bytes) != 16 {
		return "", fmt.Errorf("invalid byte array length")
	}
	...
	// Data1
	for i := 0; i < 4; i++ {
		parts[0] += fmt.Sprintf("%02x", bytes[3-i])
	}

	// Data2
	for i := 0; i < 2; i++ {
		parts[1] += fmt.Sprintf("%02x", bytes[5-i])
	}

	// Data3
	for i := 0; i < 2; i++ {
		parts[2] += fmt.Sprintf("%02x", bytes[7-i])
	}

	// Data4
	parts[3] = fmt.Sprintf("%02x%02x", bytes[8], bytes[9])

	// Data5
	for i := 0; i < 6; i++ {
		parts[4] += fmt.Sprintf("%02x", bytes[10+i])
	}
